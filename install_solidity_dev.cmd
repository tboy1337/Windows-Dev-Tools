...
:skip_foundry
echo [2/7] Installing/Updating Solar...
echo.
powershell -NoProfile -ExecutionPolicy Bypass -Command "try { Invoke-RestMethod https://paradigm.xyz/solar/install.ps1 | Invoke-Expression; exit 0 } catch { Write-Host \"ERROR: $_\"; exit 1 }" 2>&1
if %errorlevel% neq 0 (
    echo WARNING: Failed to install Solar.  Error code: %errorlevel%
...
echo [3/7] Installing/Updating Slither...
%PYTHON_CMD% -m pip install --upgrade slither-analyzer
echo.

echo [4/7] Installing/Updating solc-select...
%PYTHON_CMD% -m pip install --upgrade solc-select
echo.

echo [5/7] Installing/Updating Code Quality Tools...
call npm install -g prettier prettier-plugin-solidity
echo.

echo [6/7] Installing/Updating Solhint...
call npm install -g solhint
echo.

echo [7/7] Installing/Updating Halmos...
%PYTHON_CMD% -m pip install --upgrade halmos
echo.
